...
class SCRationaleHandlera(
  private val context: Activity
) : RationaleHandler(context, context::askForPermissions) {
  private var dialog: MaterialDialog? = null

  init {
    onPermission(WRITE_EXTERNAL_STORAGE, R.string.storage_permission_prompt_d_capture)
  }
  ...
  override fun showRationale(
    ...
    if (permission == WRITE_EXTERNAL_STORAGE) {
      dialog = MaterialDialog(context).show {
        title(R.string.storage_permission_prompt_capture)
